...
- **Render serves the JS assets gzip-compressed.** A plain `curl -s …/assets/index-<hash>.js | grep "<string>"` searches raw gzip bytes, finds nothing, and (under zsh) errors with `character not in range`. To smoke-test the *contents* of a deployed bundle (e.g. confirm a new i18n string shipped), fetch with `curl -s --compressed`. The bundle-hash check on `index.html` above works without it because the hash is plainly present, but content greps inside `/assets/*.js` need `--compressed`. Note bundled locale strings live in the main `index-<hash>.js` chunk, not a separate `.json` asset, so grep that chunk.
...
- **`verify_jwt` is per-function and NOT stored in `config.toml`** here — it's set by the deploy flag and sticks until the next deploy. Current state: `chat` = false (`--no-verify-jwt`), `devotional` = false (`--no-verify-jwt`), `send-push` = **true** (deployed with NO flag; it's gateway-protected and called server-side). **Footgun:** redeploying `send-push` with `--no-verify-jwt` would silently open it to the public. `supabase functions list` does NOT show the verify_jwt state in CLI v2.98.2, so probe it empirically before redeploying: a no-auth `curl` returns gateway `401 UNAUTHORIZED_NO_AUTH_HEADER` when verify_jwt=true, or reaches the function (e.g. 400/405) when false.
